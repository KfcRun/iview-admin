import {getContentByMsgId, getMessage, getUnreadCount, hasRead, removeReaded, restoreTrash} from '@/api/user'

import {deleteToken, parseToken, postToken, verifyCode} from '@/api/token'
...
export default {
  ...
  actions: {
    // 获取验证码
    handleVerify({commit}, {principal, credential}) {
    ...
    // 登录
    handleLogin({commit}, {principal, credential, vcode}) {
      principal = principal.trim()
      credential = credential.trim()
      return new Promise((resolve, reject) => {
        postToken({
